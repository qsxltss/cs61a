    ...
    def schedule_helper(galaxy, sum_sofar, index):
        ...
        if galaxy[index] == '?': 
          for x in range(0,max_digit+1):
            if index == 0:modified_galaxy = str(x)+galaxy[1:]
            elif index == len(galaxy)-1: modified_galaxy=galaxy[0:len(galaxy)-1]+str(x)
            else: modified_galaxy=galaxy[0:index]+str(x)+galaxy[index+1:len(galaxy)]
            ans+=schedule_helper(modified_galaxy,sum_sofar+x,index+1)
        else:
          ans+=schedule_helper(galaxy,sum_sofar+int(galaxy[index]),index+1)
        return ans
