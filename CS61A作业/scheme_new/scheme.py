...
def eval_all(expressions, env):
    ...
    #print(first)
    while(rest.rest != nil):
        ans = scheme_eval(rest.first,env)
        rest = rest.rest
    return scheme_eval(rest.first,env,True) 
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        #新frame的parent是这里的self
        f =self
        new_frame = Frame(f)
        x,y = formals,vals
        if len(x) != len(y):
            print("长度有问题--make_child_frame")
            raise SchemeError
        while x is not nil:
            new_frame.define(x.first,y.first)
            x = x.rest
            y = y.rest
        return new_frame
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        x = args
        while x is not nil:
            python_args.append(x.first)
            x = x.rest
        # If self.use_env is True, then add the current 
        # environment env as the last argument to this Python list
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        #相较于简单版本多了个省略
        #拼成lambda需要的expressions
        formals = target.rest
        body = expressions.rest
        expressions_new = Pair(formals,body)
        ans = do_lambda_form(expressions_new,env)
        #增加env
        env.define(target.first, ans)
        #(list (list 'lambda (g values nil) (car body)) (f values nil))
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while rest != nil:
        ...
        names = Pair(rest.first.first,names)
        #if isinstance(rest.first.rest.first, int) or isinstance(rest.first.rest.first, float): 
        #    values = Pair(rest.first.rest.first,values)
        if self_evaluating(rest.first.rest.first):
            values = Pair(rest.first.rest.first,values)
        else:
            values = Pair(scheme_eval(rest.first.rest.first,env),values)
        rest = rest.rest
    if validate_formals(names) == False:
        raise SchemeError
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        body = expressions.rest
        #expressions_new = Pair(formals,body)
        ans = MacroProcedure(formals,body,env)
        #增加env
        env.define(target.first, ans)
        return target.first
    else:
...
def optimize_tail_calls(prior_eval_function):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result,Thunk):
            #print(scheme_eval(result.expr,result.env))
