...
def subseqs(s):
    ...
    if s==[]:
        ...
        new_seq = [s[i] for i in range(1,len(s))]
        p=subseqs(new_seq)
        return insert_into_all(s[0],p)+p
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return subseq_helper(s0,prev)
        else:
            a = s[0]
            b = [s[i] for i in range(1,len(s))]
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    total_points,points_earned=0,0
    def func(key_word,num):
      nonlocal total_points
      nonlocal points_earned
      total_points += class_assignments.get(key_word)
      points_earned += num
...
def make_advanced_counter_maker():
    ...
    def func1():
        cnt_count = 0      
        def func2(words):
            nonlocal cnt_count,cnt_global_count
            if words == "count":
              cnt_count+= 1
              return cnt_count
            elif words == 'reset': 
              cnt_count= 0 
            elif words =='global-count':
              cnt_global_count+= 1
              return cnt_global_count
            else:
              cnt_global_count= 0
        return func2
    return func1
