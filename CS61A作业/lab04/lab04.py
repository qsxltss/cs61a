...
def reverse_iter(lst):
    ...
    for i in range(0,length0):
      lst[i],lst[len(lst)-1-i]=lst[len(lst)-i-1],lst[i]
    return lst
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x1,x2,y1,y2=get_lat(city_a),get_lat(city_b),get_lon(city_a),get_lon(city_b)
    l1=(x1-x2)*(x1-x2)
    l2=(y1-y2)*(y1-y2)
    return sqrt(l1+l2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    l1=distance(city0,city_a)
    l2=distance(city0,city_b)
    if(l1<=l2):return get_name(city_a)
