...
def in_order_traversal(t):
    ...
    "*** YOUR CODE HERE ***"
    ans = []
    if t.is_leaf(): ans.append(t.label)
    else:
        ans1 = in_order_traversal(t.branches[0])
        for x in ans1:
            ans.append(x)
        ans.append(t.label)
        if(len(t.branches)>1):
            ans2 = in_order_traversal(t.branches[1])
            for x in ans2:
                ans.append(x)
    return ans
...
def mutate_reverse(link):
    ...
    values = [link.first]
    l1 = link
    while l1.rest is not Link.empty:
        l1 = l1.rest
        values.append(l1.first)
    i = 0
    ans = Link(values[i])
    i = i+1
    while i < len(values):
        ans = Link(values[i],ans)
        i+=1
    link.first = ans.first
    link.rest = ans.rest
