...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if self.contain_check == True and self.can_contain(place.ant)==True:
              self.contained_ant=place.ant
              place.ant =self
            elif place.ant.contain_check == True and place.ant.can_contain(self) == True:
              place.ant.contain_ant(self)
              #place.ant = self
            else: assert place.ant is None, 'Two ants in {0}'.format(place)
            # END Problem 9
            #print(place.ant)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        def func(self):
          nonlocal place_for_change,distance
          while place_for_change.entrance!=None and distance<=self.max_range:
            if place_for_change.bees!=[] and distance >=self.min_range:
              return rANTdom_else_none(place_for_change.bees)
            place_for_change= place_for_change.entrance
            distance+=1
          return None
        return func(self)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        place_there = self.place
        damage_current=self.damage
        bees_now= []
        if self.armor>amount:
          for x in self.place.bees:
            if x.armor>amount:
              ...
              x.place =None
              x.death_callback()
            #Insect.reduce_armor(x,amount)
          #for x in bees_now:
            #if x.place != None: bees_now+=[x]
        else:
          for x in self.place.bees:
            if x.armor>amount+self.damage:
              x.armor=x.armor - amount -self.damage
              bees_now+=[x]
            else:
              x.place = None
              x.death_callback()
        self.place.bees = bees_now
...
class ContainerAnt(Ant):
    def __init__(self, *args, **kwargs):
        ...
        self.contain_check = True

    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant ==None and other.contain_check == False:return True
        else: return False
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.already_has_queen == False:
          QueenAnt.already_has_queen = True
          self.already_has_queen = True
          self.True_queen = True
        #print(self.True_queen)
    ...
    def action(self, gamestate):
        ...
        if self.True_queen == False:
          ...
          while place_to_check.exit!=None:
            place_to_check=place_to_check.exit
            if place_to_check.ant != None and place_to_check.ant.doubled_by_queen == False:
              place_to_check.ant.damage *=2
              place_to_check.ant.doubled_by_queen = True
            if place_to_check.ant != None and place_to_check.ant.contain_check == True and place_to_check.ant.contained_ant !=None and place_to_check.ant.contained_ant.doubled_by_queen == False:
              place_to_check.ant.contained_ant.damage *=2
              place_to_check.ant.contained_ant.doubled_by_queen = True
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
          self.armor = 0
          if self.place.ant == self: self.place.ant = None
          else: self.place.ant.contained_ant =None
          self.place = None
          self.death_callback()
          if self.True_queen == True: bees_win()
        else:
          self.armor -= amount
        # END Problem 13
