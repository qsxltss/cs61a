...
def make_withdraw(balance, password):
    ...
    error_times=0
    error_try=[]
    def func(amount,passwords_try):
      nonlocal error_times,error_try,balance
      if error_times>=3:
        str = "Too many incorrect attempts. Attempts: ["
        for i in range(0,len(error_try)-1):
          str+="'"
          str+=error_try[i]
          str+="'"
          str+=", "
        str+="'"
        str+=error_try[len(error_try)-1]
        str+="'"
        str+="]"
        return str
      if passwords_try != password:
        error_times+=1
        error_try+=[passwords_try]
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(check) == str: return check
    else:
      def joint_withdraw(amount,passwords_try):
        nonlocal old_pass,new_pass
        if passwords_try == new_pass: 
          return withdraw(amount,old_pass)
        else:
          return withdraw(amount,passwords_try)
